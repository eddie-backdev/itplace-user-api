...
**현재 성능 테스트 범위는 웹 전용이다.** 후속 지시에 따라 모바일 API를 제외했다. 아래 혼합 결과는 모바일 5%가 포함된 과거 비교로 보존하며 현재 웹 전용 성능으로 제시하지 않는다. 후속 변경의 [웹 전용 혼합 비교](web-preview-renewal-2026-09-11.md)는 975.22→949.09 TPS, 평균 508.08→522.42ms로 속도 향상을 입증하지 못했다. 동일 104개 질의의 JSON body는 50.0% 줄고 검색 누락은 감소했다. 아래 지도 전용 결과는 처음부터 모바일 요청이 없었다. [현재 입력 생성·실행](../../scripts/loadtest/MapLatencyRunner.md).
