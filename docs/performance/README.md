...
**현재 대상은 지도 이동의 viewport preview·cluster 두 API뿐이다.** 키워드·주변 검색·모바일을 제외한 [지도 이동 전용 소규모 재확인](map-movement-only-2026-09-11.md)을 따른다. 기존 [웹 혼합 지연 진단](map-root-causes-2026-09-11.md)의 키워드 우선 개선 판단은 이번 범위에서 철회했다. 혼합 부하에서 관측한 대기를 지도 이동만의 원인으로 사용하지 않는다.

2026-09-12 사용자 요청으로 [100 VUser·5분 지속 부하](map-movement-100vu-5min-2026-09-12.md)를 확인했다. 예열 30초 후 300초 동안 **8,686.33 TPS, 평균 11.380ms, p95 26.988ms, 2,605,980건 오류 0**이었다. 16만 고유 좌표를 순환한 로컬 공유 호스트 결과이며, 정기 DB 집계 재계산은 제외했다. 새 코드의 전후 개선률이나 클라우드 최대 용량으로 해석하지 않는다.

**다음 수치는 키워드·주변 검색을 포함한 과거 웹 혼합 시험이다.** 당시 [회귀 수정 후 웹 혼합 재검증](web-preview-regression-2026-09-11.md)은 기준 `b9401d8` 대비 `df4ff46`에서 **1,014.43→1,140.37 TPS(+12.42%), 평균 488.88→435.71ms(-10.88%)**다. 로컬 500개 동시 요청·전후 3회, 294,697건 오류 0이며 모든 쌍에서 개선됐다. 앞선 [975.22→949.09 TPS 중간 실험](web-preview-renewal-2026-09-11.md)은 실패 기록으로 보존한다. 아래 혼합 결과는 모바일 5%를 포함한 더 이전 비교이며 이 웹 결과와 연결하지 않는다. 아래 지도 전용 결과는 처음부터 모바일 요청이 없었다. [현재 지도 전용 입력 생성·실행](../../scripts/loadtest/MapLatencyRunner.md).
...
2026-09-11 후속 작업으로 웹 매장 유효성 검사의 정규화를 쓰기 시점으로 옮기고, 주변·검색 응답에도 compact를 적용했다. [추가 변경·별도 웹 전용 검증](web-preview-renewal-2026-09-11.md)과 [LIMIT 전 유효성 검사](map-read-eligibility-2026-09-11.md)를 참고한다. 아래 기존 수치는 추가 작업 전의 기록이다.
