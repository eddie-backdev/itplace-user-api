...
현재 남은 지연의 원인과 후속 구조는 [TPS·평균 지연 진단 및 지도 서비스 비교](map-root-causes-2026-09-11.md)에 따로 정리했다. 기존 500부하의 DB 연결 대기, 클라이언트/Spring 관측 시간 차이, 위치 없는 ES 검색과 DB 보충을 구분하고 Mapbox·Google·Kakao·Uber의 공식 공개 방식과 비교한다. 이 문서의 제안을 이미 구현한 개선으로 집계하지 않는다.

**현재 성능 테스트 범위는 웹 전용이다.** 후속 지시에 따라 모바일 API를 제외했다. 최신 [회귀 수정 후 웹 혼합 재검증](web-preview-regression-2026-09-11.md)은 기준 `b9401d8` 대비 `df4ff46`에서 **1,014.43→1,140.37 TPS(+12.42%), 평균 488.88→435.71ms(-10.88%)**다. 로컬 500개 동시 요청·전후 3회, 294,697건 오류 0이며 모든 쌍에서 개선됐다. 앞선 [975.22→949.09 TPS 중간 실험](web-preview-renewal-2026-09-11.md)은 실패 기록으로 보존한다. 아래 혼합 결과는 모바일 5%를 포함한 과거 비교이며 새 웹 결과와 연결하지 않는다. 아래 지도 전용 결과는 처음부터 모바일 요청이 없었다. [현재 입력 생성·실행](../../scripts/loadtest/MapLatencyRunner.md).
