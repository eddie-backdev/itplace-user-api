...
> 이 단계는 응답속도 개선의 완료 조건을 통과하지 못한 중간 실험이다. 500개 동시 요청의 지연 증가·처리량 감소를 응답량 감소로 대신 정당화하지 않는다. 원본 비교는 회귀 원인 분석과 후속 수정의 기준으로 보존한다.

후속 자료: [SQL 회귀 원인과 최소 수정](web-preview-sql-regression-2026-09-11.md), [동일 웹 입력의 재검증](web-preview-regression-2026-09-11.md).
