...
지도 미리보기의 결과 변환 비용과 넓은 영역 조회의 잘못된 실행 계획을 줄이고, 키워드 검색이 모든 매장을 반복 검사하는 경로를 정리했다. 이 문서는 **baseline `86cdc70`과 개선 작업 트리의 SQL 비교 및 정확성 검사**를 기록한다. 해당 단계의 전국 좌표 HTTP 비교는 [통합 기록](README.md), 이후 정규화·compact 추가 개선은 [별도 웹 전용 기록](web-preview-renewal-2026-09-11.md)을 따른다.
