...
현재 성능 테스트 대상은 웹 API다. 모바일 구현·정합성 검사 기록은 남기되 향후 부하에서 제외한다. 모바일 5%가 포함된 기존 혼합 609→1,043 TPS를 웹 전용 성능으로 인용하지 않는다. 아래 6개 사례의 웹 지도 근거는 모바일이 없었던 상세·클러스터 4,357→5,497 TPS, 평균 114→91ms다. [후속 사례 7·8](web-preview-portfolio-cases-2026-09-11.md)의 웹 혼합 비교는 975.22→949.09 TPS, 508.08→522.42ms로 속도 향상을 입증하지 못했다. 응답량 감소·검색 누락 방지 성과와 구분한다.
