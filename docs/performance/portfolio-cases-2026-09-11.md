...
현재 성능 테스트 대상은 웹 API다. 모바일 구현·정합성 검사 기록은 남기되 향후 부하에서 제외한다. 모바일 5%가 포함된 기존 혼합 609→1,043 TPS를 웹 전용 성능으로 인용하지 않는다. 아래 6개 사례의 웹 지도 근거는 모바일이 없었던 상세·클러스터 4,357→5,497 TPS, 평균 114→91ms다. [후속 사례 7·8과 회귀 수정](web-preview-portfolio-cases-2026-09-11.md)은 중간 실험의 속도 저하를 수정한 뒤 새 웹 혼합 비교에서 1,014.43→1,140.37 TPS, 평균 488.88→435.71ms를 확인했다. 각 단계의 기준과 입력 범위를 구분한다.
...
- 후속 사례: [정규화의 쓰기 시점 이동과 LIMIT 전 검사](map-read-eligibility-2026-09-11.md), [검색·주변 공통 혜택 응답과 별도 웹 전용 검증](web-preview-renewal-2026-09-11.md). 기존 6개 사례의 통합 성능에 나중 변경을 소급해서 귀속하지 않는다.
- [후속 사례 7·8 발표 원고](web-preview-portfolio-cases-2026-09-11.md)는 문제·선택·검증·한계와 면접 설명을 별도로 정리한다.
